      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.88rem' }}>
        <thead>
          <tr>
            ...
            <th style={th}>Melhor Custo</th>
          </tr>
        </thead>
        <tbody>
          {produtos.map(p => {
            let menor = Infinity, mercMenor = '-', infoMenor = null
            let melhorCusto = Infinity, mercMelhorCusto = '-', infoMelhorCusto = null
            let unidadeBase = ''
            ...
            mercados.forEach(m => {
              ...
              if (info && info.preco && p.quantidade) {
                const qtd = parseFloat(p.quantidade)
                const un = p.unidade || 'un'
                const precoPorUnidade = calcularPrecoPorUnidade(info.preco, qtd, un)
                if (precoPorUnidade && precoPorUnidade.valor < melhorCusto) {
                  melhorCusto = precoPorUnidade.valor
                  mercMelhorCusto = m
                  infoMelhorCusto = info
                  unidadeBase = precoPorUnidade.unidade || un
                }
              }
            })

            const precoPorUnidadeStr = formatarPrecoPorUnidade(
              menor !== Infinity ? menor : null,
              p.quantidade,
              p.unidade || 'un'
            )
              ...
              <tr key={p.id}>
                <td style={td}>
                  <div style={{ display: 'flex', alignItems: 'start', gap: 6 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      {precoPorUnidadeStr && menor !== Infinity && (
                        <div style={{ fontSize: '0.7rem', color: '#64748b', marginTop: 2 }}>
                          💰 {precoPorUnidadeStr}
                        </div>
                      )}
                    </div>
                    {(onEditarProduto || onRemoverProduto) && (
                      <div style={{ display: 'flex', gap: 2, flexShrink: 0 }}>
                        {onEditarProduto && (
                          <button onClick={() => onEditarProduto(p)} style={iconBtn} title="Editar produto">✏️</button>
                        )}
                        {onRemoverProduto && (
                          <button onClick={() => onRemoverProduto(p)} style={{ ...iconBtn, color: '#ef4444' }} title="Remover produto">🗑️</button>
                        )}
                      </div>
                    )}
                  </div>
                ...
                {mercados.map(m => {
                  ...
                  if (v && p.quantidade) {
                    const qtd = parseFloat(p.quantidade)
                    const un = p.unidade || 'un'
                    const resultado = calcularPrecoPorUnidade(v, qtd, un)
                    if (resultado) {
                      precoUnidadeDisplay = `${resultado.valor.toFixed(2).replace('.', ',')}/${resultado.unidade}`
                    }
                  }
                  ...
                  return (
                    <td key={m} style={td}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'flex-end' }}>
                        {mine ? (
                          <input
                            ...
                            style={{
                              width: 85,
                              padding: '6px 8px',
                              border: `1px solid ${best ? '#10b981' : '#93c5fd'}`,
                              borderRadius: 6,
                              textAlign: 'right',
                              fontSize: '0.85rem',
                              background: best ? '#ecfdf5' : '#eff6ff',
                          ...
                          <div
                            title="Somente quem cadastrou esse mercado pode editar"
                            style={{
                              width: 85,
                              padding: '6px 8px',
                              border: `1px solid ${best ? '#10b981' : '#e2e8f0'}`,
                              borderRadius: 6,
                              textAlign: 'right',
                              fontSize: '0.85rem',
                        ...
                        {precoUnidadeDisplay && v && (
                          <div style={{
                            fontSize: '0.6rem',
                            color: bestCusto ? '#059669' : '#94a3b8',
                            fontWeight: bestCusto ? 700 : 400,
                            marginTop: 1
                          }}>
                            {precoUnidadeDisplay}
                            {bestCusto && ' 🏆'}
                        ...
                        {info?.oferta && (
                          <span title={info.obsOferta || `Preço com ${info.tipoOferta}`} style={selo}>
                            🏷️ {info.tipoOferta}
                          </span>
                        )}
                        {mine && v && onEditarOferta && (
                          <button onClick={() => onEditarOferta(p)} style={linkBtn}>
                            {info?.oferta ? 'alterar oferta' : 'marcar oferta'}
                          </button>
                        )}
                      </div>
                    </td>
                ...
                <td style={{ ...td, color: '#059669', fontWeight: 700, background: '#ecfdf5' }}>
                  ...
                  {infoMenor?.oferta && (
                    <div style={{ fontSize: '0.7rem', fontWeight: 600, color: '#b45309' }} title={infoMenor.obsOferta}>
                      🏷️ exige {infoMenor.tipoOferta}
                    </div>
                  )}
                </td>
                <td style={{ ...td, color: '#059669', fontWeight: 700, background: '#f0fdf4' }}>
                  {melhorCusto !== Infinity ? (
                    <>
                      <span style={{ fontSize: '0.82rem' }}>
                        {formatarMoeda(melhorCusto)}/{unidadeBase || 'un'}
                      </span>
                      {mercMelhorCusto !== '-' && (
                        <div style={{ fontSize: '0.72rem', fontWeight: 400, color: '#047857' }}>
                          {mercMelhorCusto}
                          {mercMelhorCusto === mercMenor && ' (também mais barato)'}
                        </div>
                      )}
                      {infoMelhorCusto?.oferta && (
                        <div style={{ fontSize: '0.65rem', fontWeight: 600, color: '#b45309' }}>
                          🏷️ exige {infoMelhorCusto.tipoOferta}
                        </div>
                      )}
                    </>
                  ) : '—'}
